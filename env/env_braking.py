...
class AccEnv(gym.Env):
    ...
    def __init__(self, seed):
        ...
        self.observation_space = spaces.Box(
            low=np.array([7.0, 0.0]), # [target velocity, current velocity, detected, security distance, current distance] in m/s
            high=np.array([carla_utility.compute_security_distance(180) + 21, self.RADAR_RANGE]), # [target velocity, current velocity, detected, security distance, current distance] in m/s
            dtype=np.float32
        )
