...
class GymEnv(gym.Env):
    ...
    def _compute_reward(self, observation):
        ...
        isEgoMaintainSpeed = absolute_speed - 1 <= ego_velocity <= absolute_speed + 1
        ...
        if isObjectDetected:
            ...
            if distance > security_distance:
                if isEgoStopped:
                    ...
                    reward -= (distance - security_distance) / distance
            elif isEgoInSecurityDistanceRange:
                if isEgoMaintainSpeed: 
                    reward += 10.0
                else:
                    reward -= abs(ego_velocity - absolute_speed) * 0.1
            elif isEgoTooClose: 
                reward -= (security_distance - distance) / security_distance
