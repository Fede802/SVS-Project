...
def draw_radar_point(radar: carla.Actor, point: carla.RadarDetection, color = carla.Color(0, 255, 0)):
    ...
    x = point.depth * math.cos(point.azimuth) * math.cos(point.altitude)
    y = point.depth * math.sin(point.azimuth) * math.cos(point.altitude)
    z = -point.depth * math.cos(point.azimuth) * math.sin(point.altitude)
    print("[DEBUG]: x=", x, "y=", y, "z=", z, "alt = ", __rotate(shifted_max_range_point, point.azimuth, point.altitude))
    debug.draw_line(radar_location, __add(from_np(__rotate(shifted_max_range_point, point.azimuth, point.altitude)),radar_location), life_time=4, color=color)
...
def draw_radar_bounding_box(radar: carla.Actor):
    ...
    debug.draw_line(radar_location, __add(from_np(shifted_max_range_point), radar_location), life_time=60)
    debug.draw_line(radar_location, __add(from_np(__rotate(shifted_max_range_point, radar_h_fov, radar_v_fov)),radar_location), life_time=60)
    debug.draw_line(radar_location, __add(from_np(__rotate(shifted_max_range_point, radar_h_fov, -radar_v_fov)),radar_location), life_time=60)
    debug.draw_line(radar_location, __add(from_np(__rotate(shifted_max_range_point, -radar_h_fov, radar_v_fov)),radar_location), life_time=60)
    debug.draw_line(radar_location, __add(from_np(__rotate(shifted_max_range_point, -radar_h_fov, -radar_v_fov)),radar_location), life_time=60)
    #radar.get_world().tick()
...
def evaluate_point(radar: carla.Actor, point: carla.RadarDetection):
    ...
    x = point.depth * math.sin(point.altitude) * math.cos(point.azimuth)
    y = point.depth * math.sin(point.altitude) * math.sin(point.azimuth)
    z = point.depth * math.cos(point.altitude)

    print(x, y, z, x_min < x < x_max and y_min < y < y_max and z_min < z < z_max)
